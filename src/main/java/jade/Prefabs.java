...
public class Prefabs {
    ...
    public static GameObject generateQuestionBlock() {
        ...
        GameObject questionBlock = generateSpriteObject(items.getSprite(10), 0.25f, 0.25f);

        AnimationState run = new AnimationState();
        run.title = "Flicker";
        float defaultFrameTime = 0.23f;
        run.addFrame(items.getSprite(10), 0.57f);
        run.addFrame(items.getSprite(11), defaultFrameTime);
        run.addFrame(items.getSprite(12), defaultFrameTime);
        run.setLoop(true);
